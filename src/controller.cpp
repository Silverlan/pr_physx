...
pragma::physics::PhysXController &pragma::physics::PhysXController::GetController(IController &c) { return *static_cast<PhysXController *>(c.GetUserData()); }
...
pragma::physics::IShape *pragma::physics::PhysXController::GetGroundShape() const { return (m_pGroundTouchingHit && m_pGroundTouchingHit->shape.expired() == false) ? &PhysXShape::GetShape(*m_pGroundTouchingHit->shape.lock()) : nullptr; }
...
pragma::physics::IMaterial *pragma::physics::PhysXController::GetGroundMaterial() const { return IsTouchingGround() ? m_pGroundTouchingHit->material.lock().get() : nullptr; }
...
void pragma::physics::PhysXController::MoveController(const Vector3 &displacement, bool testOnly)
{
	...
	m_controller->move(GetPxEnv().ToPhysXVector(displacement), 0.f, GetPxEnv().GetNetworkState().DeltaTime(), filters);
	m_controller->getState(m_controllerState);
}
void pragma::physics::PhysXController::DoMove(Vector3 &disp) { MoveController(disp, false); }
void pragma::physics::PhysXController::SetPos(const Vector3 &pos) { m_controller->setPosition(GetPxEnv().ToPhysXExtendedVector(pos)); }
Vector3 pragma::physics::PhysXController::GetPos() const { return GetPxEnv().FromPhysXVector(m_controller->getPosition()); }
void pragma::physics::PhysXController::SetFootPos(const Vector3 &footPos) { m_controller->setFootPosition(GetPxEnv().ToPhysXExtendedVector(footPos)); }
Vector3 pragma::physics::PhysXController::GetFootPos() const { return GetPxEnv().FromPhysXVector(m_controller->getFootPosition()); }
void pragma::physics::PhysXController::SetUpDirection(const Vector3 &up) { m_controller->setUpDirection(GetPxEnv().ToPhysXNormal(up)); }
Vector3 pragma::physics::PhysXController::GetUpDirection() const { return GetPxEnv().FromPhysXNormal(m_controller->getUpDirection()); }
void pragma::physics::PhysXController::SetSlopeLimit(umath::Degree slopeLimit) { m_controller->setSlopeLimit(umath::cos(umath::deg_to_rad(slopeLimit))); }
umath::Degree pragma::physics::PhysXController::GetSlopeLimit() const { return umath::rad_to_deg(umath::acos(m_controller->getSlopeLimit())); }
void pragma::physics::PhysXController::SetStepHeight(float stepHeight) { m_controller->setStepOffset(stepHeight); }
float pragma::physics::PhysXController::GetStepHeight() const { return m_controller->getStepOffset(); }
	...
	case physx::PxControllerShapeType::eBOX:
		{
			...
			extents = {controller.getHalfForwardExtent(), controller.getHalfHeight(), controller.getHalfSideExtent()};
			break;
		}
	case physx::PxControllerShapeType::eCAPSULE:
		{
			auto &controller = static_cast<physx::PxCapsuleController &>(*m_controller);
			extents = {controller.getRadius(), controller.getHeight(), controller.getRadius()};
			break;
		}
	}
...
void pragma::physics::PhysXController::Resize(float newHeight) { m_controller->resize(newHeight); }
...
physx::PxControllerBehaviorFlags pragma::physics::CustomControllerBehaviorCallback::getBehaviorFlags(const physx::PxShape &shape, const physx::PxActor &actor) { return physx::PxControllerBehaviorFlag::eCCT_CAN_RIDE_ON_OBJECT; }

physx::PxControllerBehaviorFlags pragma::physics::CustomControllerBehaviorCallback::getBehaviorFlags(const physx::PxController &controller) { return physx::PxControllerBehaviorFlag::eCCT_CAN_RIDE_ON_OBJECT; }

physx::PxControllerBehaviorFlags pragma::physics::CustomControllerBehaviorCallback::getBehaviorFlags(const physx::PxObstacle &obstacle) { return physx::PxControllerBehaviorFlag::eCCT_CAN_RIDE_ON_OBJECT; }
