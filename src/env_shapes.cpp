...
{
	auto geometry = std::shared_ptr<physx::PxGeometry>(new physx::PxCapsuleGeometry {static_cast<physx::PxReal>(ToPhysXLength(halfWidth)), static_cast<physx::PxReal>(ToPhysXLength(halfHeight))}, [](physx::PxGeometry *p) { delete p; });
	if(geometry == nullptr)
		return nullptr;
...
{
	auto geometry = std::shared_ptr<physx::PxGeometry>(new physx::PxBoxGeometry {ToPhysXVector(halfExtents)}, [](physx::PxGeometry *p) { delete p; });
	if(geometry == nullptr)
		return nullptr;
	return CreateSharedPtr<PhysXConvexShape>(*this, geometry);
}
...
{
	auto geometry = std::shared_ptr<physx::PxGeometry>(new physx::PxSphereGeometry {static_cast<float>(ToPhysXLength(radius))}, [](physx::PxGeometry *p) { delete p; });
...
std::shared_ptr<pragma::physics::IShape> pragma::physics::PhysXEnvironment::CreateHeightfieldTerrainShape(uint32_t width, uint32_t length, Scalar maxHeight, uint32_t upAxis, const IMaterial &mat) { return nullptr; }
